(function (global, factory) {
  ...
  function isALinkTo(path, element) {
    ...
    var broken = element.getAttribute('href').split('?')[0]
    var value = decodeURIComponent(broken) === decodeURIComponent('#' + path);
    console.log("is a link to: " + value)
    return value
  }
  ...
  function pagination(vm, crossChapter) {
    try {
      ...
      var close = componentClosest(active, 'ul')
      var group = toArray((close || {}).children).filter(function (element) {
